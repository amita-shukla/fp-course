...
(*>) ::
  ...
  -> f b
  -> f b
-- (*>) a b = const id <$> a <*> b -- here (const id) is a function that takes any thing gibberish but always returns id
                                -- map id over a, then apply it on b
                                -- the function here looks similar... err to lift 2
(*>) = lift2 (const id)
